...
const fbBot = () => {
  login({ email: 'sn@snstudio.xyz', password: '1qa2ws3ed' }, (e1, api) => {
    if (e1) {
    ...
    // api.getThreadList(0, 10, 'pending', (e12, id) => {
    //   threadid = id;
    // });
    // api.handleMessageRequest(threadid, true, (e11) => {
    //   if (e11) {
    //     console.log(`e11:${e11}`);
    //   }
    // });
    ...
    api.listen((e2, message) => {
      ...
      if (e2) {
        ...
        if (msg === '/') {
          ...
          api.sendMessage('正在為您查詢中，請稍候...', message.threadID);
          try {
            const battletag1 = msg.split(' ')[1];
            const battletag2 = msg.split(' ')[2];
        ...
        } else if (msg === '/SNstudio') {
          api.sendMessage('主人什麼的最棒了XDDDD', message.threadID);
        } else if (msg === '/bt') {
          api.sendMessage(`範例battletag:\n${battletags}`, message.threadID);
        } else if (msg === '/openkick') {
          if (message.senderID === seanID) {
            flag = 1;
            api.sendMessage('opened', message.threadID);
          } else {
            api.sendMessage('你沒有權限使用', message.threadID);
          }
        } else if (msg === '/closekick') {
          if (message.senderID === seanID) {
            flag = 0;
            api.sendMessage('closed', message.threadID);
          } else {
            api.sendMessage('你沒有權限使用', message.threadID);
          }
        } else if (msg === '/kick') {
          if (message.senderID === seanID || flag === 1) {
